...
# ==========================================
# 1. LÓGICA DE CLASES Y FUNCIONES (POO)
# ==========================================
@dataclass
class Columna:
    id: str
...
def calcular_centros_cortante(datos_pisos, total_pisos):
    ...
    # Bucle descendente (del último piso al primero)
    for p in range(total_pisos, 0, -1):
        ...
        # 2. Suma de momentos estáticos respecto al origen
        sum_Fy_Xg += (Fy * CMx)
        sum_Fx_Yg += (Fx * CMy)
        
        # 3. Ubicación del Centro de Cortante (V)
...
# --- ÁREA PRINCIPAL: CONFIGURACIÓN POR PISOS ---
...
for i, tab in enumerate(tabs):
    ...
    with tab:
        ...
        with col_datos:
            ...
            # Ya no pedimos Cortante Acumulado, sino Fuerza de Piso individual
            ...
            # Pedimos explícitamente el Centro de Masas físico del piso
        ...
        if len(columnas_activas) > 0:
            ...
            # Guardamos los datos de ingreso
...
if st.button("Procesar Análisis Torsional para todos los pisos", type="primary", use_container_width=True):
    ...
    # Se calcula la bajada de cargas para obtener el Centro de Cortante
    ...
    for piso, datos in resultados_por_piso.items():
        ...
        # Recuperar Cortantes Acumulados y Centros de Cortante calculados
        c_piso = cortantes_globales[piso]
        
        # 1. Extraemos el DataFrame de columnas activas en este piso
        df_cols_actual = datos["cols"]
        
        # 2. NUEVO: Calculamos las dimensiones reales (locales) de la planta en este nivel
        ...
        try:
            # 3. Pasamos Dx_local y Dy_local en lugar de los globales
            ...
            # --- MÉTRICAS PREVIAS Y VALOR 'J' ---
            st.markdown("Cálculos Previos")
            c_m, c_c, c_r = st.columns(3)
            
            # --- CONTINÚA EL RESTO DEL CÓDIGO IGUAL ---
